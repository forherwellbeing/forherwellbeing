...
export default function StaffOverview({ T }) {
  ...
  // Monthly revenue bar chart — last 6 months from real data
  const monthlyRevenue = Array.from({ length: 6 }, (_, i) => {
    ...
    return { l: label, v: v || 0 }
  })

  return (
    <div>
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:20 }}>
        <StatCard icon="♀" label="Total Patients"       value={loading ? '…' : String(total)}                                       change="All time"       color={T.accent}  />
        <StatCard icon="◷" label="Today's Appointments" value="—"                                                                    change="Connect calendar" color={T.info}    />
        <StatCard icon="₹" label="Total Revenue"        value={loading ? '…' : `₹${(revenue/1000).toFixed(1)}K`}                    change="All payments"   color={T.success} />
        <StatCard icon="✦" label="Active Programs"      value={loading ? '…' : String(Object.keys(programCounts).length)}           change="Running now"    color={T.warning} />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 316px', gap:14 }}>
        <div style={{ background:'#fff', borderRadius:16, padding:22, boxShadow:'0 2px 14px rgba(0,0,0,0.05)' }}>
          ...
          <div style={{ fontSize:12, color:'#7A7A8A', marginBottom:14 }}>Last 6 months (₹)</div>
          {loading
