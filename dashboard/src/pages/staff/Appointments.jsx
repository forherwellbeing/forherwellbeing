...
const buildWaLink = a => {
  ...
  const time = fmtTime(a.appointment_time)
  let msg = `Hello ${a.patient_name}, your consultation with Dr. Raga Deepthi is confirmed for ${date} at ${time}.`
  if (a.meet_link) msg += ` Please join using this link: ${a.meet_link}`
  msg += ` Be ready 5 minutes early. — For Her Wellbeing`
                ...
                <tr key={a.id} style={{ borderBottom:'1px solid #EDE8E5' }}>
                  ...
                  <td style={{ padding:'12px 15px' }}>
                    <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                      {buildWaLink(a) && (
                        <a href={buildWaLink(a)} target="_blank" rel="noreferrer"
                          style={{ display:'inline-flex', alignItems:'center', gap:5, padding:'5px 11px', borderRadius:8, background:'#E7F8F0', color:'#1A6640', fontSize:12, fontWeight:600, textDecoration:'none', border:'none', cursor:'pointer' }}>
                          <svg width={13} height={13} viewBox="0 0 24 24" fill="#25D366"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.554 4.118 1.528 5.849L0 24l6.335-1.508A11.95 11.95 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.808 9.808 0 0 1-5.006-1.371l-.36-.214-3.728.887.936-3.618-.235-.372A9.797 9.797 0 0 1 2.182 12C2.182 6.57 6.57 2.182 12 2.182c5.43 0 9.818 4.388 9.818 9.818 0 5.43-4.388 9.818-9.818 9.818z"/></svg>
                          Send
                        </a>
                      )}
