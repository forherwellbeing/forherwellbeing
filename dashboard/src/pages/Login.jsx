...
export default function Login({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', background:'#FAF8F5' }}>
      ...
      {/* Right sign-in panel */}
      <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding: isMobile ? '40px 24px' : 44 }}>
        {isMobile && (
          <div style={{ textAlign:'center', marginBottom:32 }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:24, fontWeight:700, color:'#1A1A2E', lineHeight:1.2 }}>
              For Her <span style={{ fontStyle:'italic', color:T.accent }}>Wellbeing</span>
            </div>
            <div style={{ fontSize:11, color:'#7A7A8A', letterSpacing:'0.08em', textTransform:'uppercase', marginTop:6 }}>Healthcare Management Portal</div>
          </div>
        )}
        <div style={{ width:'100%', maxWidth:400 }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <input value={email} onChange={e => setEmail(e.target.value)} type="email" placeholder="Email address" style={{ width:'100%', border:'1.5px solid #EDE8E5', borderRadius:10, padding:'11px 13px', fontSize:14, background:'#FAFAFA', color:'#1A1A2E', outline:'none', boxSizing:'border-box' }} />
          </div>
          <div style={{ marginBottom:8 }}>
            <label style={{ display:'block', fontSize:13, fontWeight:500, color:'#1A1A2E', marginBottom:5 }}>Password</label>
            <input value={password} onChange={e => setPass(e.target.value)} type="password" placeholder="••••••••" style={{ width:'100%', border:'1.5px solid #EDE8E5', borderRadius:10, padding:'11px 13px', fontSize:14, background:'#FAFAFA', color:'#1A1A2E', outline:'none', boxSizing:'border-box' }} onKeyDown={e => e.key === 'Enter' && handleLogin()} />
          </div>
